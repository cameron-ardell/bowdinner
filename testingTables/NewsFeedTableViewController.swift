...
class NewsFeedTableViewController: UITableViewController, OtherProfileDataSource {
    ...
    func profileInfo(name: [String]) -> [String]? {
        for i in 0...allFriendsArray.count-1 {
            if allFriendsArray[i][0] == name[0] && allFriendsArray[i][1] == name[1] {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
       ...
       // cell.statusInformation = [individual[0], individual[1], individual[2], individual[3]]
        ...
        cell.personName = [individual[0], individual[1]]
        
        cell.userName.text = "\(individual[0]) \(individual[1])"
        
        
        if individual[2] == "unavailable" || individual[3] == "unavailable" {
            cell.userAvailability.text = "Unavailable"
            cell.userLocation.text = " "
            cell.userTime.text = " "
        } else {
            cell.userAvailability.text = "Available"
            cell.userLocation.text = "\(individual[4])"
            cell.userTime.text = "\(individual[2]) to \(individual[3])"
        }
